...
let $results := (
  ...
  return
  if ($i/js:array[@key='isSourceCodeOf']) then
    for $item in $i/js:array[@key='isSourceCodeOf']/*
    let $consumesFormats :=
  ...
  else if ($i/js:map[@key='isSourceCodeOf']) then
    let $consumesFormats :=
      for $product in $i/js:map[@key='isSourceCodeOf']/js:*[@key='consumesData']
      return $product/js:*[@key='inLanguage']/js:*[@key='name']
    let $producesFormats :=
      for $prod in $i/js:map[@key='isSourceCodeOf']/js:*[@key='producesData']
